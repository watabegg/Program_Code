...
void forward_step_wise_selection(int nf, double x[N], double y[N], int n, char* path, char* data)
    ...
    for (k=1; k <= nf; k++)     /* k basic functions */
    {
        ...
        for (i=1; i<= nf; i++)
        {
            if (f_in_model(current_m, i) == FALSE)
            {
                ...
                /* モデルの誤差を求める　*/
                current_m.err = model_error(n, k , x, y, current_m.sol, current_m.f_id);
        ...
        model_error_output(fp_out, k, best_m.err, best_m.k, best_m.sol, best_m.f_id);
...
void forward_step_wise_selection_train_test(int nf, double x[N], double y[N], int n, char* path, char* data, double x_test[N], double y_test[N], int n_test)

{
    ...
    for (k=1; k <= nf; k++)     /* k basic functions */
    {
        ...
        /*　モデルの誤差, モデルの基本関数と求めた係数を出力 */
        test_error = model_error(n_test,best_m.k , x_test, y_test, best_m.sol, best_m.f_id);

        model_train_test_error_output(fp_out, k, best_m.err, best_m.k, best_m.sol, best_m.f_id, test_error);
