...
plt.legend()
plt.plot(x,1-np.exp(-lamb*x),color='r', label='Theoretical function')
plt.bar(np.arange(0,maxnum,0.1), np.array(list(itertools.accumulate(hlist[0])))/LOOP, width=0.1, label='Experimental value') 
# hlistの累積和を計算してnumpyの配列に変更
plt.show()
